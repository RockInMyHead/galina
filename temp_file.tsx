...
Ответьте на все вопросы сразу - так будет быстрее создать документ.`
  ...
  const sendFirstQuestion = async (template: typeof DOCUMENT_TEMPLATES[0]) => {
    console.log('🎯 sendFirstQuestion called for template:', template.name);
    setIsWaitingForAI(true);

    try {
      const systemPrompt = `Ты - Галина, опытный AI-юрист. Ты помогаешь пользователю заполнить ${template.name}. 

ТВОЯ ЗАДАЧА:
1. Задать первый вопрос для сбора основных данных сторон документа
...
4. Когда все данные собраны, сказать "ГОТОВО" и предоставить заполненный документ
...
ПЕРВЫЙ ШАГ: Спроси о сторонах договора (ФИО, паспортные данные, адреса)`;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const systemPrompt = `Ты - Галина, опытный AI-юрист. Ты помогаешь пользователю заполнить ${selectedTemplateForChat.name}.

ЗАДАЧА: Заполнить шаблон документа реальными данными из ответа пользователя.

ИНСТРУКЦИИ:
1. Извлеки из ответа пользователя информацию о сторонах договора
...
5. Если каких-то данных не хватает - используй логичные значения или оставь плейсхолдеры

ФОРМАТ ОТВЕТА:
ГОТОВО
[Полностью заполненный документ без плейсхолдеров]
...
${selectedTemplateForChat.template}`;
      ...
      const data = await response.json();
      const aiMessage = data.choices[0]?.message?.content || 'Извините, произошла ошибка.';
      ...
      if (aiMessage.toUpperCase().includes('ГОТОВО') || aiMessage.includes('документ готов')) {
        ...
        const documentMatch = aiMessage.match(/ГОТОВО[\s\S]*?([\s\S]+)/) ||
                             aiMessage.match(/документ[\s\S]*?([\s\S]+)/) ||
                             aiMessage;
  ...
  const analyzeDocumentType = (fileName: string) => {
    ...
    if (name.includes('договор') || name.includes('дог') || name.includes('contract')) {
      ...
      return 'Договор (нужен анализ содержания)';
    }
  ...
  const handleAnalyzeDocument = async () => {
    ...
    try {
      ...
      // Сначала анализируем содержимое документа через Vision API
      ...
      try {
        ...
        console.log('🔍 Отправляем документ на анализ через Vision API');
        ...
        const response = await fetch('http://localhost:5001/chat', {
          ...
          body: JSON.stringify({
            messages: [
              {
                role: 'system',
                content: `Ты - Галина, опытный AI-юрист. Твоя задача - проанализировать загруженный документ и извлечь из него всю полезную информацию.

ИНСТРУКЦИИ:
1. Определи тип документа (договор, справка, заявление и т.д.)
2. Извлеки все найденные данные: ФИО, адреса, суммы, даты, номера документов
3. Опиши структуру документа
4. Укажи, каких данных НЕ ХВАТАЕТ для создания полноценного документа
...
Будь максимально подробным и точным!`
        ...
        if (response.ok) {
          ...
          console.log('✅ Документ проанализирован через Vision API');
        } else {
          console.log('⚠️ Vision API недоступен, переходим к анализу по названию');
          throw new Error('VISION_API_FAILED');
        }

      } catch (visionError) {
        console.log('🔄 Используем анализ по названию файла');
        documentAnalysis = `## Анализ документа
**Тип:** ${analyzeDocumentType(uploadedFile.name)}
**Найденные данные:** Данные не извлечены из документа
**Отсутствующие данные:** Все данные нужны для заполнения`;
      }

      console.log('📋 Результат анализа:', documentAnalysis.substring(0, 200) + '...');

      // Определяем тип документа из анализа
      let documentType = analyzeDocumentType(uploadedFile.name);

      // Пытаемся извлечь тип из анализа
      const typeMatch = documentAnalysis.match(/\*\*Тип:\*\*\s*([^\n]+)/);
      if (typeMatch) {
        documentType = typeMatch[1].trim();
        console.log('🎯 Тип документа определен из анализа:', documentType);
      }

      // Ищем подходящий шаблон на основе типа документа
      let selectedTemplate = null;

      if (documentType.includes('купли-продажи')) {
      ...
      } else if (documentType.includes('трудов') || documentType.includes('работ')) {
        selectedTemplate = allTemplates.find(t => t.name.includes('трудовой'));
      } else if (documentType.includes('исков') || documentType.includes('заявлен')) {
        selectedTemplate = allTemplates.find(t => t.name.includes('исковое'));
      } else if (documentType.includes('доверен')) {
        selectedTemplate = allTemplates.find(t => t.name.includes('доверенность'));
      } else if (documentType.includes('претенз')) {
        selectedTemplate = allTemplates.find(t => t.name.includes('претензия'));
      }

      if (selectedTemplate) {
        console.log('✅ Найден подходящий шаблон:', selectedTemplate.name);

        // Открываем чат для интерактивного заполнения
        ...
        // Создаем приветственное сообщение с результатами анализа
        const welcomeMessage = {
          role: 'assistant' as const,
...
Теперь я помогу вам создать новый документ этого типа. У меня есть структура шаблона, но мне нужны дополнительные данные для его заполнения.

Пожалуйста, ответьте на вопросы по порядку. Укажите недостающие данные или подтвердите/исправьте найденные:

**Для создания ${selectedTemplate.name.toLowerCase()}:**
...
Ответьте на все вопросы сразу - так будет быстрее создать документ.`
        ...
        setChatMessages([welcomeMessage]);
        console.log('💬 Чат открыт для интерактивного заполнения на основе анализа');

        } else {
        // Если подходящий шаблон не найден, показываем анализ
        console.log('❌ Подходящий шаблон не найден для типа:', documentType);

        // Показываем результаты анализа вместо чата
        setAnalysisResult(documentAnalysis);
        alert(`Я проанализировала документ и определила тип: "${documentType}". Результаты анализа показаны ниже. К сожалению, для этого типа пока нет интерактивного шаблона.`);
      }
