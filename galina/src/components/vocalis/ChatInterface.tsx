...
import { useInterval } from '../../utils/vocalis/hooks';
import BackgroundStars from './BackgroundStars';
import AssistantOrb from './AssistantOrb';
import websocketService, { MessageType, ConnectionState } from '../../services/vocalis/websocket';
import audioService, { AudioEvent, AudioState } from '../../services/vocalis/audio';
...
const ChatInterface: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleTTSStart = (data: any) => {
      console.log('TTS start received, transitioning to speaking state');
      setAssistantState('speaking');
    };

    const handleTTSEnd = (data: any) => {
      console.log('TTS end received, transitioning to idle state');
      setAssistantState('idle');

      // End greeting flow protection once the initial greeting has finished playing
      if (assistantState === 'speaking' && isFirstInteraction) {
        console.log('Initial greeting playback complete, ending greeting flow protection');
        websocketService.setGreetingFlowState(false);
    ...
    const handleTTSAudio = (data: any) => {
      if (data.audio_data) {
        audioService.playAudioChunk(data.audio_data, data.format || 'wav');
      }
    };

    websocketService.addEventListener(MessageType.TTS_START, handleTTSStart);
    websocketService.addEventListener(MessageType.TTS_END, handleTTSEnd);
    websocketService.addEventListener('tts_chunk', handleTTSChunk);
    websocketService.addEventListener('tts_audio', handleTTSAudio);
    ...
    return () => {  
      ...
      websocketService.removeEventListener(MessageType.TTS_START, handleTTSStart);
      websocketService.removeEventListener(MessageType.TTS_END, handleTTSEnd);
      websocketService.removeEventListener('tts_chunk', handleTTSChunk);
      websocketService.removeEventListener('tts_audio', handleTTSAudio);
      ...
      <div className="flex-1 flex items-center justify-center">
        ...
        <div className="flex flex-col items-center justify-center px-4">
          ...
          <div className="relative h-12 mt-2">
            ...
            `}>
              <div className="flex items-center gap-1 bg-blue-900/30 backdrop-blur-sm px-2 py-1 rounded-full border border-blue-400/20">
            ...
            `}>
              <div className="flex items-center gap-1 bg-slate-900/50 backdrop-blur-sm px-2 py-1 rounded-full">
            ...
            `}>
              <div className="flex items-center gap-1 bg-amber-900/30 backdrop-blur-sm px-2 py-1 rounded-full border border-amber-400/20">
            ...
            `}>
              <div className="flex items-center gap-1 bg-emerald-900/30 backdrop-blur-sm px-2 py-1 rounded-full border border-emerald-400/20">
            ...
            `}>
              <div className="flex items-center gap-1 bg-red-900/30 backdrop-blur-sm px-2 py-1 rounded-full border border-red-400/20">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3">
          ...
          <button
            ...
            className={`
              ...
              ${assistantState === 'vision_file'
                ? 'bg-sky-400/40 hover:bg-sky-400/60 border-sky-300/70 shadow-sky-400/20' // Sky blue for vision file state - brighter
                : assistantState === 'vision_asr'
                  ? 'bg-emerald-500/40 hover:bg-emerald-500/60 border-emerald-400/70 shadow-emerald-500/20' // Emerald for vision ASR - brighter
                : assistantState === 'vision_processing'
                  ? 'bg-teal-500/40 hover:bg-teal-500/60 border-teal-400/70 shadow-teal-500/20 cursor-not-allowed' // Teal for vision processing - brighter
                  : assistantState === 'listening'
                    ? 'bg-indigo-500/40 hover:bg-indigo-500/60 border-indigo-400/70 shadow-indigo-500/20' // Purple-blue for listening - brighter
                    : assistantState === 'greeting'
                      ? 'bg-indigo-500/40 hover:bg-indigo-500/60 border-indigo-400/70 shadow-indigo-500/20 cursor-not-allowed' // Purple-blue for greeting - brighter
                      : assistantState === 'processing'
                        ? 'bg-purple-500/40 border-purple-400/60 shadow-purple-500/20 cursor-not-allowed' // Purple for processing (disabled) - brighter
                        : assistantState === 'speaking'
                          ? 'bg-indigo-500/30 hover:bg-indigo-500/50 border-indigo-400/60 shadow-indigo-500/20' // Purple-blue for speaking - brighter
                          : isFirstInteraction
                            ? 'bg-indigo-900/30 border-indigo-400/20 cursor-not-allowed opacity-50' // Disabled state before call starts - brighter
                            : 'bg-indigo-900/50 hover:bg-indigo-900/70 border-indigo-400/60 shadow-indigo-400/20' // Default for idle - brighter
              }
              border-2 shadow-xl backdrop-blur-md transform transition-all duration-300
              hover:scale-105
            `}
        ...
        <button
          ...
          className={`
            p-4 rounded-full transition-all duration-300
            ${assistantState === 'listening'
              ? 'bg-emerald-500/40 hover:bg-emerald-500/60 border-emerald-400/70 shadow-emerald-500/20' // Green for listening - brighter
              : assistantState === 'greeting'
                ? 'bg-blue-500/40 hover:bg-blue-500/60 border-blue-400/70 shadow-blue-500/20 cursor-not-allowed' // Blue for greeting - brighter
                : assistantState === 'processing'
                  ? 'bg-purple-500/40 border-purple-400/60 shadow-purple-500/20 cursor-not-allowed' // Purple for processing - brighter
                  : assistantState === 'speaking'
                    ? 'bg-amber-500/30 hover:bg-amber-500/50 border-amber-400/60 shadow-amber-500/20' // Amber for speaking - brighter
                    : !isFirstInteraction && assistantState === 'idle'
                      ? 'bg-red-500/30 hover:bg-red-500/50 border-red-400/60 shadow-red-500/20' // Red tint for end call - brighter
                      : 'bg-slate-700/50 hover:bg-slate-800/70 border-slate-400/60 shadow-slate-400/20' // Default for idle - brighter
            }
            border-2 shadow-xl backdrop-blur-md transform transition-all duration-300
            ${isFirstInteraction && assistantState === 'idle' 
        ...
        <button
          ...
          className={`
            ...
            ${isFirstInteraction
              ? 'bg-indigo-900/30 border-indigo-400/20 cursor-not-allowed opacity-50' // Disabled state before call starts - brighter
              : isMuted
                ? 'bg-red-500/40 hover:bg-red-500/60 border-red-400/70 shadow-red-500/20' // Red for muted - brighter
                : 'bg-emerald-500/40 hover:bg-emerald-500/60 border-emerald-400/70 shadow-emerald-500/20' // Green for unmuted - brighter
            }
            border-2 shadow-xl backdrop-blur-md transform transition-all duration-300
