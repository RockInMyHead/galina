...
const AssistantOrb: React.FC<AssistantOrbProps> = ({ state }) => {
  ...
  return (
    ...
    >
      ...
      {/* Aurora effect */}
      <div className={`
        absolute inset-0 rounded-full shadow-lg shadow-primary/10
        backdrop-blur-md overflow-hidden border
        ring-1
        ${state === 'vision_file' ? 'bg-gradient-to-b from-primary/20 via-primary/15 to-primary/10 border-primary/30 ring-primary/20' : ''}
        ${state === 'vision_processing' ? 'bg-gradient-to-b from-blue-400/25 via-blue-300/20 to-blue-400/25 border-blue-400/30 ring-blue-400/20' : ''}
        ${state === 'vision_asr' ? 'bg-gradient-to-b from-green-400/25 via-green-300/20 to-green-400/25 border-green-400/30 ring-green-400/20' : ''}
        ${state === 'listening' ? 'bg-gradient-to-b from-green-400/25 via-green-300/20 to-green-400/25 border-green-400/30 ring-green-400/20' : ''}
        ${state === 'greeting' ? 'bg-gradient-to-b from-primary/25 via-primary/20 to-primary/25 border-primary/30 ring-primary/20' : ''}
        ${state === 'processing' ? 'bg-gradient-to-b from-purple-400/20 via-blue-400/15 to-purple-400/20 border-purple-400/25 ring-purple-400/15' : ''}
        ${state === 'speaking' ? 'bg-gradient-to-b from-orange-400/20 via-yellow-400/15 to-orange-400/20 border-orange-400/25 ring-orange-400/15' : ''}
        ${state === 'idle' ? 'bg-gradient-to-b from-primary/20 via-slate-300/15 to-primary/20 border-primary/25 ring-primary/15' : ''}
      `}>
        <canvas
          ref={canvasRef}
          width={300}
          height={300}
          className="absolute inset-0 w-full h-full opacity-90 backdrop-blur-sm"
        />
      </div>
    </div>
  );
