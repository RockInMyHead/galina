...
interface ChatMessageProps {
  ...
  isLast?: boolean
  onPersonalDataConsent?: (consented: boolean) => void
}

export const ChatMessage = ({ message, isLast: _isLast, onPersonalDataConsent }: ChatMessageProps) => {
    ...
    <div className={`flex items-start gap-3 ${isAssistant ? '' : 'justify-end'}`}>
      ...
      <div className={`flex-1 ${isAssistant ? '' : 'max-w-[70%]'}`}>
        <div className={`rounded-lg p-4 ${
          ...
          {/* Personal data consent action */}
          {isAssistant && message.requiresAction && message.actionType === 'personal_data_consent' && onPersonalDataConsent && (
            <div className="flex gap-2 mt-3 pt-3 border-t border-border/50">
              <Button
                size="sm"
                variant="default"
                onClick={() => onPersonalDataConsent(true)}
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => onPersonalDataConsent(false)}
