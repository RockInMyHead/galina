# 🚀 API сервер Галиной

## Быстрый запуск

```bash
cd /Users/artembutko/Desktop/Galina/api

# Диагностика проблем
npm run diagnose

# Установка зависимостей (если нужно)
npm install

# Инициализация базы данных (если нужно)
npm run db:push

# Запуск сервера
npm start
```

## 📋 Скрипты

- `npm start` - Запуск с диагностикой (рекомендуется)
- `npm run direct` - Прямой запуск без диагностики
- `npm run diagnose` - Диагностика проблем
- `npm run db:push` - Инициализация базы данных

## 🔧 Переменные окружения

Создайте файл `.env.local`:

```env
PORT=5001
DATABASE_URL="file:./prisma/galina.db"
OPENAI_API_KEY=your_openai_api_key_here
OPENAI_BASE_URL=https://api.openai.com/v1
```

## 🚨 Возможные проблемы

### 1. "Не удалось подключиться к серверу"
- Сервер не запущен
- Проверьте порт 5001: `curl http://localhost:5001/health`

### 2. Ошибки зависимостей
```bash
rm -rf node_modules package-lock.json
npm install
```

### 3. Проблемы с базой данных
```bash
npm run db:push
```

### 4. Порт занят
```bash
lsof -i :5001  # Проверить, кто занимает порт
kill -9 <PID>  # Освободить порт
```

## ✅ Проверка работы

```bash
curl http://localhost:5001/health
# Должен вернуть: {"status":"ok","message":"Galina API is running"}
```

## 🔗 Эндпоинты

- `GET /health` - Проверка состояния сервера
- `POST /chat` - Чат с ИИ
- `POST /tts` - Текст в речь
- `POST /stt` - Речь в текст

## 📊 Мониторинг

Сервер логирует:
- ✅ Запуск и инициализация
- ✅ Подключения к базе данных
- ✅ Ошибки API запросов
- ✅ Состояние здоровья


