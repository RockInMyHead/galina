...
>(({ className, children, ...props }, ref) => {
  // Проверяем наличие DialogTitle для accessibility
  const hasDialogTitle = React.useMemo(() => {
    return React.Children.toArray(children).some(
      ...
      (child.type === DialogTitle ||
       ...
       // Проверяем на sr-only класс
       (child.props && child.props.className && child.props.className.includes('sr-only') &&
        React.Children.toArray(child.props.children).some(
          (grandChild) => React.isValidElement(grandChild) && grandChild.type === DialogTitle
        )))
    );
  ...
  // Если нет DialogTitle, добавляем его автоматически
  const contentWithTitle = hasDialogTitle ? children : (
    <>
